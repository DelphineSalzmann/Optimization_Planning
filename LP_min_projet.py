...
def LP_nb_projet(nom_instance, Gmin):
    ...
    # def fini_rule(model,p,h):
    #     return (sum(model.a[t,s,q,p] for s in model.S for q in model.Q for t in RangeSet(1,h)))/(sum(model.n[p,q] for q in model.Q)) >=model.f[p,h]
    # model.fini = Constraint(model.P, model.H, rule=fini_rule, doc='projet fini')
    ...
    def profit_rule(model):
        return sum(model.f[p]*model.g[p]-model.R[p]*model.r[p] for p in model.P)>=Gmin
    model.profit=Constraint(rule=profit_rule, doc="profit minimal")
    ...
    #Objective
    model.obj=Objective(expr=sum(model.k[s,p] for s in model.S for p in model.P), sense=minimize)
